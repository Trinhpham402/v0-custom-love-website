  ...
  useEffect(() => {
    const checkAndLoadMusic = async () => {
      try {
        ...
        audio.src = "/music/background.mp3?" + Date.now() // Cache busting
        audio.load()
    ...
    const handleFirstInteraction = async () => {
      if (hasMusic && audioRef.current && !isPlaying) {
        try {
          ...
          // Tự động phát nhạc thất bại, cần nhấn nút play
        }
      }
    ...
    return (
      <div className="fixed bottom-4 right-4 z-50">
        ...
        <div className="absolute bottom-full right-0 mb-2 bg-red-600 text-white text-xs p-3 rounded max-w-xs">
          ...
          <div className="text-xs opacity-75 mb-2">Kiểm tra: public/music/background.mp3</div>
          <button
